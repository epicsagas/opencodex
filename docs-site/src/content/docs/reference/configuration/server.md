...
| `updateCheck?` | `boolean` | `true` | Automatic update checks: the npm registry version lookups behind the interactive `ocx start` update prompt and its 20h background refresh. Explicit `ocx update` and the GUI update button are unaffected. Environment opt-out: `OPENCODEX_UPDATE_CHECK=0`. |
